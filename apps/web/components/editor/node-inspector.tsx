...
export function NodeInspector({ definitions, providers }: { definitions: readonly NodeDefinitionDto[]; providers: readonly ProviderDescriptor[] }) {
  ...
  const parameters = configRecord(config.parameters);
      ...
      <div className="inspector-scroll">
        ...
        {aiNode ? (
          <>
            ...
            {config.providerId === 'comfyui' ? <section className="inspector-section"><h3>ComfyUI workflow</h3><label>parameters.workflow JSON<textarea className="textarea code-textarea" rows={8} defaultValue={JSON.stringify(parameters.workflow ?? {}, null, 2)} onBlur={(event) => { try { const workflow: unknown = JSON.parse(event.target.value); set({ parameters: { ...parameters, workflow } }); setParameterError(null); } catch { setParameterError('Workflow must be valid JSON.'); } }} /></label>{parameterError ? <p className="form-error">{parameterError}</p> : null}</section> : null}
